...
def graph(kind="attention", mode="training"):
    ...
    if mode == "training":
        for key, x, title, detail in [
            ...
            ("tokenize",640,"Tokenize + assign IDs","train-only vocabulary"),
    ...
    if attention:
        ...
        edge("qkv","message",[(455,316),(455,342),(1270,342),(1270,316)],"V bypasses scoring","v",True)
    ...
    if mode == "training":
        # The MLP path snakes back across the bottom row, so logits reach the
        # loss vertically. The observed target has its own outer-right lane.
        xs = [40,440,840,1240] if attention else [1240,840,440,40]
        node("loss",xs[0],bottom,"Cross-entropy(z, y)","one target per window","a")
        node("backward",xs[1],bottom,"loss.backward()","gradients for θ","q")
        node("optimizer",xs[2],bottom,"optimizer.step()","update learned θ","q")
        node("parameters",xs[3],bottom,"Next training batch","reuse updated θ","q")
        if attention: edge("logits","loss",[(155,481),(155,bottom)])
        else: edge("logits","loss",[(1355,316),(1355,bottom)])
        if attention:
            edge("windows","loss",[(1470,73),(1490,73),(1490,bottom-32),(280,bottom-32),(280,bottom+43),(270,bottom+43)],"target y [B]","a",True,(1120,bottom-42))
        else:
            edge("windows","loss",[(1470,73),(1490,73),(1490,bottom+43),(1470,bottom+43)],"y [B]","a",True,(1440,365))
...
def pipeline_svg(kind="attention", mode="training", focus=(), *, region=None, uid=None):
    ...
    prefix='pipeline-'+(uid or f'{kind}-{mode}-'+('-'.join(sorted(active)) or 'all'))
    if not all(c.isalnum() or c in '-_' for c in prefix):
        raise ValueError('Diagram UID must contain only letters, digits, hyphens or underscores')
    markers=''.join(f'<marker id="{prefix}-{role}-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M0 0 L10 5 L0 10z" fill="{colors[0]}"/></marker>' for role,colors in COLORS.items())
    out=[f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="{" ".join(map(str,view))}" role="img" aria-label="{kind} {mode} pipeline" data-focus="{" ".join(sorted(active))}" style="font-family:Avenir Next,Segoe UI,sans-serif;background:#F7F8FA">',
         '<defs>'+markers+'</defs>']
    ...
    for e in g["edges"]:
        ...
        stroke=COLORS[e["role"]][0];opacity=1 if not active or e["start"] in active or e["end"] in active else .35
        d="M"+" L".join(f"{x} {y}" for x,y in points)
        out.append(f'<g opacity="{opacity}" data-edge="{e["start"]}:{e["end"]}"><path d="{d}" fill="none" stroke="{stroke}" stroke-width="2.5" marker-end="url(#{prefix}-{e["role"]}-arrow)"'+(' stroke-dasharray="7 5"' if e["dashed"] else '')+'/>')
        if e["label"]:
    ...
    for n in g["nodes"]:
        ...
        out.append(f'<g data-stage="{n["key"]}" data-active="{str(on).lower()}" opacity="{1 if on else .45}"><rect x="{n["x"]}" y="{n["y"]}" width="{n["w"]}" height="86" rx="9" fill="{fill}" stroke="{stroke}" stroke-width="{4 if active and on else 2}"/>')
    ...
    if mode == 'training':
        out.append(f'<text x="760" y="{g["height"]-10}" text-anchor="middle" font-size="22" fill="#4A5160">The next batch repeats the forward pass with updated parameters in every learned layer.</text>')
