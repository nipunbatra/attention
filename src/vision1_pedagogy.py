...
def expand(b, sections):
    ...
    revisions={
      ...
      'why-cls':('We have several patch rows. Where does the answer go?','CLS is one extra row that collects information for the image label.'),
    ...
    revised=[
      ...
      ('What does each row know about its patch?',seq('s02-small','position-question','s02-projection','patch-matrix','empty-patch','mean-loses-edge','edge-filters','two-identical-patches','s02-positions')),
      ('How does one row read the other rows?',seq('why-cls','image-mask','qkv-roles','all-qkv','q-dot','one-key-dot','s03-query','one-score','softmax-relative','s03-weights','weight-denominator','s03-values','one-value-product','weight-message','change-query')),
