...
Opening five frames: two Part 1 recaps (the actual aab → i example, three two-coordinate embeddings, concatenation and the 6→32 ReLU→27 predictor; then training/generation and the fixed-window limitation), followed by three paper frames (Vaswani et al. 2017 with the first-page screenshot, why direct connections and parallel training mattered with BERT/GPT links, and the original encoder-decoder Figure 1 as an orientation). Images open at full resolution and retain attribution. Distinguish the original translation model from this lecture's causal teaching model. Stable IDs are `s01-recap-model`, `s01-recap-context`, `s01-paper-intro`, `s01-paper-importance`, and `s01-paper-architecture`.

Then "One blank": sentence chips and the prediction task. Show probability bars. Distinguish E_tok from a looked-up row. Explain the four illustrative word features with actual river/bank/the rows and matching colour labels. Motivate order with Maya/Ravi and bridge from Part I's concatenation. Add actual word/position rows at positions 1, 5, and 10, without an extra axis. Assemble the ten-by-four sentence matrix and define T and d_model. Keep technical position details in the article companion, not repeated recap slides.
  Notes: "What could come next, and why do you think so?" Collect answers before switching context. The selected context and token survive continuation frames. Unpack attention only after motivating the context problem.
