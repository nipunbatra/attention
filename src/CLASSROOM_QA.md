...
- Added the complete paper first-page screenshot and original Figure 1 as local, attributed assets. Both open at full resolution. Caption and provenance identify arXiv revision v7 separately from the 2017 publication date. Primary paper links support the introductory claims. The presentation skill guided the native tables and existing slide typography; the humanizer pass kept the copy direct and the importance claims specific. The PDF skill guided rendering and visual inspection of the paper screenshot.
- Added two worked softmax slides before the saturation explanation. They use the same raw scores [-8, 8] and scaled scores [-1, 1] as the comparison. Each reveals both exponentials, their common sum, and the two divisions, decimal probabilities and percentages. The notes distinguish rounded display values, full-precision calculation, Euler's number, score scaling, saturation and numerical overflow.
- New opening regression checks the Part 1 dimensions against its saved model, source images and attribution, reveal order, navigation, the starting deep link, unchanged model output, and desktop/tall/phone containment. Updated the position-intro route expectations, including the existing baseline topic break. Opening and position checks pass. The scaling suite independently verifies every exponential, denominator, weight and percentage, as well as forward/reverse math visibility. Token-flow and 36-frame tail checks pass. Full-deck audit: 272 states / 547 formulas, no math failures or overflow. Visually inspected all five opening slides, phone architecture layout and both worked softmax slides. No numerical model, shared-runtime, dependency or other lecture changes.
...
- The presentation skill guided the paired native tables, fixed row positions, matched query/key/value colours and progressive value reveals. Both examples are explicitly illustrative. Notes explain that these are later-layer input representations, not initial embeddings or measured model outputs. The second Maya follows its described states and prior action, and coat follows all its listed attributes, so the candidate source positions do not need future information. The field labels sketch shared-width numerical vectors, not different literal coordinate types. Kept the later eight-dimensional numerical video example unchanged.
- Extended role checks verify all eight labelled facts, identical source rows and prefix across queries, source selection, past-only fact placement, the caveats, value reveals and bidirectional navigation. Search-flow regression includes the new frame in the 21-frame section and retains the original numerical retrieval results. Its responsive check now waits for reading layout to settle after a viewport change, avoiding an intermediate desktop-width measurement while still enforcing phone containment. Visually checked desktop and phone tables; phone checks include cell-level overflow. Full audit: 241 states / 539 formulas, no math errors or overflow. No numerical model, shared-runtime, dependency or other lecture changes.
...
- The presentation skill guided the example-first sequence, native vectors/tables, matched role colours and staged explanation. Independent-reference checks cover both contexts' queries, source keys/values, contributions, messages, updates and prediction results, including attention bypass and restoration. The bypass still yields identical predictions for the two contexts. Extended reveal checks inspect every mathematical leaf, catching a Chromium/reduced-motion visibility problem in dynamically rendered subscripts; a Section 14-only style keeps them synchronized on forward and backward reveals.
- Visually inspected both classroom frames, context changes and phone reading. The 26-frame tail regression checks frame order, all reveal states, reverse navigation, controls and phone containment. Full audit: 238 states / 539 formulas, no math errors or overflow. No model, shared-runtime, dependency or other lecture changes.
...
- The result slide shows a worked water-coordinate calculation and all numerical rows at full opacity. Both SVG views retain their dimension labels and residual paths; wider cells accommodate negative entries. Notes distinguish a linear map into embedding space from an orthogonal projection, and explain that adding a bias would make the map affine. The presentation skill guided the existing diagram style and typography.
- Verified all model hard targets, 7,265 independent JS/Python values, 9,260 live browser values, 48 capacity/generation cases, 2,112 saved training entries and all 332 gradient directions. Lecture 3 continuity checks pass 1,578 values. The standalone 12-stage preview passes its data, visual-bounds and interaction checks. Token-flow regression checks the dense map, displayed vectors, scalar expansion, signed-number padding in both visible SVGs, navigation, zoom interactions and phone layout. The full audits pass 225 states / 517 formulas in Lecture 2 and 105 states / 156 formulas in Lecture 3, without overflow or math errors. Visually inspected the result and both SVGs; phone tables/formulas remain locally scrollable with no page-width overflow.
...
## 2026-09-21: geometric position intuition and the complete attention path

- Expanded the positional section to 45 frames. Equal-axis plots precede the addition table. Each sentence has four reversible SVG reveals, showing the exact word lookup, slot offset and positioned endpoint. The worked result follows through to a contextual update and residual addition. Position offsets are explicitly separate from both attention updates and training updates.
- Adjustable one-pair and two-pair clocks expose the illustrative collisions. Wave plots at rates 1 and 0.01 radians per slot, followed by the slow pair's full cycle, precede the standard formula. Continuous periods are distinguished from integer token indices. An absolute-addition counterexample motivates the RoPE rotation/common-shift demonstration. Visible Serrano/Huang credits remain in the topic break.
- A shared 13-node SVG closes the section with the full attention path and three focus views. It includes token and slot lookups, addition, Q/K/V shapes, score scaling, masking, attention softmax, value mixing, output projection, residual addition, a ReLU hidden layer and vocabulary softmax. All views keep the same layout. This is the additive teaching architecture, not a new RoPE implementation in the notebook.
- `check_cost_position.mjs`: 45 position frames and 164 build checks pass, including all geometric endpoints, forward/reverse word reveals, computed residual results, clock states, sine/cosine curve coordinates, additive and rotary common-shift scores, and map node/label bounds. Layout checks cover 1280×720, 1920×1080, 1024×768 and 390px reading mode. The 28 Part III cost frames still pass.
- Whole Part II audit: 394 states, 624 formulas, no math failures, console errors or overflow. Topic breaks, the concise core, unchanged live-model arithmetic and all 48 notebook pipeline frames/links pass. Inspected the new plots, clock controls and map views visually. The notebook data, trained measurements, model parameters and private Site are unchanged. Browser viewport checks do not constitute testing physical projector or AirServer hardware.
...
## 2026-09-21 · A short tokenization detour

- Inserted three frames between the authored sentence and `tokenize(sentence)`: a Tokenization topic pause, a word/character/subword comparison, and the actual notebook rules. `redder!` gives 2, 7 and 3 tokens in the three displayed schemes. The subword split is explicitly illustrative, not a trained BPE result. The comparison connects token counts to context length.
- The worked probe `Lily can't find 12 balls!` demonstrates lowercase, a straight apostrophe inside a word, a digit sequence and separate punctuation. Both models retain identical rules and a training-fitted vocabulary at generation time. Unknown tokens, loss of case/spacing and the English-only teaching scope are explicit. Background references are the Hugging Face tokenizer documentation and Sennrich et al. (2016); the concrete rules come from the existing `wordlm.py`.
- Rebuilt the 52-step notebook/guide and every downstream slide link. A clean download ran all 53 code cells. All 21 Python tests pass, and all 52 figures match the lecture as parsed SVG trees. The comparison now tolerates IPython's normalization of XML entities without ignoring text, attributes or geometry.
- Visually inspected the three classroom slides and the phone topic pause. Pipeline layout checks cover all 53 frames at 1280×720 and 1024×768, plus 390px reading containment. The topic pause has wrapping native text on phones. All 13 topic breaks pass navigation/title restoration checks. The full deck passes 399 states and 624 formulas; the unchanged model passes eight live/reference cases. No model, corpus, benchmark, dependency or private Site change.

## 2026-09-21 · Individual pairs, growing lists and tensors

- Added eight frames around the window preparation steps. An indexed story distinguishes positions from token IDs. `context_ids` and `target_id` replace `one_x` and `one_y`. Empty lists, the first pair, its two append calls, the next pair and its appends appear separately. The loop then continues from position 3, without resetting or duplicating the first two pairs.
- Both append calls and both input/target assignments are visible in slide code, not just in the notebook. Early/late-window slides display the stored rows. A dedicated conversion slide shows all seven aligned input/target rows, their integer tensor shapes, and N=7. The B=2 figure maps dataset rows 2 and 3 to batch rows 0 and 1.
- The 60-step guide, matching slide links and figures share the executable source. A freshly extracted download executes all 61 notebook code cells. All 22 Python tests pass, including intermediate list snapshots, distinct input-list objects, exact window/target alignment, unchanged toy tensors, and visible append code. All 60 notebook figures match the slide SVGs.
- Visually inspected the new classroom sequence and the batch mapping. All 61 pipeline frames pass 1280×720 and 1024×768 fit/SVG-bounds checks, with 390px reading containment. The guide passes 1280/768/390px and local links. The full Part II audit covers 407 states and 624 formulas without overflow or math errors. All 13 topic breaks and eight live/reference model cases pass. No training, benchmark, corpus, dependency or private Site changes.

## 2026-09-21 · Complete, focused and highlighted slide code

- Removed automatic first-three-line slicing, which cut the counting comprehension and a generation block. Every walkthrough stage now declares a complete excerpt or deliberately shows no code. Most excerpts have one to three lines; the complete pair-building loop retains six. Build-time checks reject invalid Python, ellipsis placeholders, missing excerpt choices, excessive lines and long lines.
- Added six focused frames: one-story counting, training-split counting, prompt-window preparation, generation logits, candidate masking/softmax and appending the chosen token. The all-splits table is now a summary without a comprehension. The generation summary explicitly repeats from the original prompt. Full notebook code still includes calculations and assertions.
- Python's standard-library tokenizer highlights fixed slide/guide code. A small text-only browser lexer keeps the same colors in both interactive steppers. No highlighting dependency or CDN is required. Checks cover complete Python in every live step, safe escaping and exact copyable text, including whitespace.
- A freshly extracted download executes all 67 notebook code cells. All 30 Python tests pass. All 66 notebook figures match their slide SVGs; chapter and slide links pass. All 67 pipeline frames pass 1280×720 and 1024×768 presentation fit and SVG bounds, plus 390px reading containment. The guide passes 1280/768/390px layouts. Visually inspected the counting, tokenization and generation slides, including 4:3 display.
- Full Part II audit: 413 states, 624 formulas, no console/math errors or overflow. All 13 topic breaks and eight live/reference model cases pass. No trained weights, benchmark evidence, corpus, other lecture parts, dependencies or private Site files changed. These viewport checks do not constitute a physical projector or AirServer test.

## 2026-09-21 · Batch IDs, token text and tensor shapes

- Expanded the batch table to six labelled columns: dataset row, batch row, X input IDs, decoded input tokens, y target ID and target token. The two selected examples keep the exact mappings `[0, 1, 8, 7]` / `<PAD> <BOS> lily found` / `5` / `a` and `[1, 8, 7, 5]` / `<BOS> lily found a` / `9` / `red`. PAD and BOS are explained below the table. Targets are observed in the story, not generated predictions.
- Added a separate `batch-ids` frame showing the integer input matrix, target vector and their shapes. Its visible explanation distinguishes completed tokenization/vocabulary lookup from later input embedding lookup. X contains two examples with four input IDs each. y contains one target ID per example and remains IDs for the loss.
- Rebuilt the 67-step guide and matching slide links. A clean download executes all 68 notebook code cells. All 31 Python tests pass, including exact visible token/ID alignment, integer dtypes and confirmation that the embedding stage has not run at this point. All 67 notebook figures match their slide assets.
- Visually inspected the two frames at widescreen and 4:3. All 68 pipeline frames pass fit and SVG-bounds checks at 1280×720 and 1024×768, plus 390px reading containment. The guide passes 1280/768/390px. Full Part II audit: 414 states and 624 formulas without overflow, console errors or math failures. Eight live/reference model cases retain the same arithmetic. No model, corpus, benchmark, dependency, other lecture part or private Site changes.

## 2026-09-21 · Humanizer pass on walkthrough headings

- Rewrote 13 long or awkward headings with the humanizer skill. Examples include "Seven examples, four batches", "Token positions and vocabulary IDs", "Observed target and model prediction" and "Per-example loss and batch mean". Clear existing headings remain. The batching explanation states the condition of one optimizer update per batch and retains the benchmark's separate sampling rule.
- Synced headings across slides, SVG accessibility labels, the notebook, study guide and manifest. All 67 executable stage blocks and their order are byte-for-byte unchanged. No numerical data or model code changed.
- All 32 Python tests pass. A freshly extracted download executes all 68 notebook code cells, and all 67 notebook figures match the slide assets. The 68 pipeline frames pass widescreen/4:3 fit and SVG bounds plus phone reading containment. The guide's layout and links pass. Full Part II audit: 414 states and 624 formulas without overflow, math errors or console errors. Inspected the revised batching and prediction slides visually.

## 2026-09-21 · Separate definitions for each dimension

- Replaced slash-paired symbols and values with two tables on the dimensions frame: B, w and C describe the data; d, h, dₖ and dᵥ describe the model widths. Every row has one symbol, one value and one definition. The text states that C includes special tokens and distinguishes query/key width from value width. No numeric values or executable stage calculations changed.
- All seven displayed values now come from the executed notebook namespace. Regression checks verify every symbol/value/definition mapping, reject slash-paired entries, and confirm that changed namespace values reach the figure. The slide, study guide, notebook and downloadable package share the same SVG.
- All 33 Python tests pass. A fresh download executes all 68 notebook code cells, and all 67 notebook figures match their slide assets. The guide's links and 1280/768/390px layouts pass. All 68 pipeline frames pass 1280×720 and 1024×768 presentation fit and SVG text bounds, plus 390px reading containment. Visually inspected the revised frame at both presentation sizes. No model, corpus, benchmark, dependency, other lecture part or private Site changes.

## 2026-09-21 · Recurring training and generation maps

- Added 20 visible map checkpoints before the relevant detailed examples, from complete stories and splitting through tokens, boundaries, windows, batching, MLP layers, attention, loss, gradients, updates and generation. Each repeats the complete diagram with fixed node locations and highlights the current operation. These checkpoints also appear as first-class sections and SVG outputs in the guide and notebook, without dummy code or parameter changes.
- Added a numeric lookup figure: X[1] contains IDs 1, 8, 7 and 5; each selects a full row from the learned T [10,4] table in slot order. It labels the four-row result E[1] and the full-batch relation X [2,4], T [10,4], E [2,4,4]. The existing batch table still decodes both X rows and y=[5,9] as a/red. All 67 original executable stage cells are unchanged.
- Simplified the MLP training route: logits enter the loss vertically, observed targets follow an outer-right lane, and a footer explains updated parameters instead of a crossing return arrow. Attention's value input and outgoing projection use distinct ports. The tokenizer node now says "assign IDs" to distinguish vocabulary lookup from learned embedding lookup. Repeated SVGs have unique, explicitly colored arrow markers so hidden frames do not suppress arrowheads.
- All 37 Python tests pass. A freshly extracted package runs all 89 notebook code cells. All 88 notebook figures match their slide assets, and guide links/layouts pass at 1280/768/390px. All 89 pipeline frames pass 1280×720 and 1024×768 fit/SVG-bounds checks and 390px reading containment. Visually inspected the story, batch, lookup, MLP, attention-message and generation maps.
- Full Part II audit: 435 states, 624 formulas, no overflow, math failures or console errors. Eight live/reference model cases pass with unchanged arithmetic. No trained model, corpus, benchmark, dependency, other lecture part or private Site changes. Classroom viewport checks are not physical projector or AirServer tests.

## 2026-09-22 · BOS and EOS belong to the complete document

- Renamed the boundary slide "One BOS and EOS per document". Its visible explanation says that a complete story receives one pair, with no extra markers between sentences. The figure labels the Lily sentence as a whole toy document, and the code explicitly uses `boundaries=True`.
- Notebook 5 includes a two-sentence example with exactly one BOS, one EOS and two full stops. Its explanation distinguishes the token-list wrapper from sentence detection and notes that other datasets may choose different conventions. The original eight toy IDs and seven targets remain unchanged.
- All 39 Python tests pass. The notebook executes all 89 code cells, and its freshly executed bytes match the downloadable ZIP. All 88 figures match their slide assets; notebook/guide links and responsive layouts pass. All 89 pipeline frames pass 1280×720 and 1024×768 fit/SVG-bounds checks and 390px reading containment. Visually inspected the revised boundary frame. No model, corpus, benchmark, dependency, other lecture part or private Site changes.

## 2026-09-24 · Position-encoding alternatives subsection

- Added a topic divider immediately before the appended-index example. The sequence now labels raw slot numbers, learned positions and sinusoidal positions as alternatives 1, 2 and 3. The opening question asks whether index scale can overwhelm word features. The worked example still explicitly says concatenation can work with suitable scaling and learned projections.
- All 18 topic breaks pass navigation, header restoration and desktop/tall/phone checks. The positional suite passes 43 frames and 146 build checks at 1280×720, 1920×1080 and 1024×768, plus 390px reading containment, with unchanged numerical results and model state.
- Full Lecture 2 audit: 430 presentation states, 649 formulas, no overflow, math failures or browser errors. Visually inspected the new divider, numbered method dividers and appended-index slide, including the user's 760×1041 viewport. No numerical code, model or notebook changes. Pending Part III edits remain outside this change.

## 2026-09-24 · Explicit identity projections for both sentences

- Added two worked matrix slides before the appended-index dot products: Maya helps Ravi today, then Ravi helps Maya today. Both show the full three-coordinate input, the 3×3 identity, and the computed 4×3 Q and K matrices in a fixed layout. The final query row is highlighted. The next slide explicitly returns to sentence 1 for its scores.
- The calculation now multiplies by explicit identity Q/K matrices with no biases. Tests check every displayed matrix entry, row label, dimension and highlighted query against independent arithmetic. Existing dot products, scores and attention weights remain unchanged. The slide distinguishes the augmented input from the two-coordinate word embedding, and states that values copy it only if W_V is also identity with no bias.
- All 45 positional frames and 152 build checks pass, including widescreen/4:3 layouts and phone reading containment. All 18 topic-break checks pass. Full Lecture 2 audit: 432 presentation states and 653 formulas, no overflow, math failures or browser errors. Visually inspected both new slides at 1280×720, 1024×768 and 760×1041. No trained model or notebook changes. Pending Part III edits remain outside this change.

## 2026-09-24 · Complete position-scale calculation walkthrough

- Added seven calculation frames around the existing dot-product and softmax slides. Both sentence orders now show Q [4,3], K transpose [3,4], every raw dot product [4,4], all four expanded word/position products for today's query, scaled and causally masked scores, each exponential and normalization division, and full attention matrices. The diagrams retain the lecture's object colours and hold the same matrix layout across sentences.
- Eight detail controls share the position scale with the final comparison. Switching between 1 and 0.1 recomputes the whole walkthrough. A separate fixed comparison shows why 4×3 becomes 0.4×0.3: shrinking both position coordinates by ten reduces their product by one hundred. The example keeps identity projections and zero biases; it does not claim that the smaller scale is optimal. Causal masking, position scaling and the separate sqrt(3) score normalization remain distinct.
- Independent checks cover every cell, word/position decomposition, masked future entries, row normalization, the exact displayed softmax divisions, shared control state, table text containment and original model immutability. All 52 positional frames and 173 build checks pass at 1280×720, 1920×1080 and 1024×768, plus 390px reading containment. All 18 topic-break checks pass. Full Lecture 2 audit: 439 states and 657 formulas with no overflow, equation failures or browser errors. Final slot-label and rounding-caption edits also pass targeted fit and SVG bounds checks.
- Visually inspected both scale settings, both sentence orders, full matrix and normalization slides, and 1024×768/760×1041 examples. Existing final-row results remain unchanged. No trained model, notebook, dependency or other lecture part changed; pending Part III edits remain separate.

## 2026-09-24 · Learned word and position tables

- Replaced the single learned-position frame with three native SVG lessons: word versus slot lookup, backpropagation and a numerical SGD step, then training coverage and table limits. The two lookup tables show C×d and Nmax×d, with Maya sharing its word row across slots 1 and 3. The explanation distinguishes a position-free lookup from other possible order cues in a causal network.
- The update diagram follows prediction loss back through addition. Both selected rows receive the whole illustrative gradient [0.2, -0.4]. With plain SGD at rate 0.1, [1,0] becomes [0.98,0.04] and [0.4,-0.2] becomes [0.38,-0.16]. Notes distinguish this single-occurrence contribution from batch gradient sums, the benchmark's AdamW optimizer, and the forward-pass attention update. The demonstration does not mutate any model or toy parameter table.
- The final frame separates a stored row without task gradients from an out-of-range lookup, identifies Nmax*d extra parameters and the absence of a built-in distance rule, and motivates a fixed encoding without promising better accuracy or length generalization. The later duplicate range example remains in reading mode, so presentation mode gains only one frame overall. Source attribution and implementation details are in the teaching notes.
- Independent arithmetic, displayed-vector, SGD and coverage checks pass, including forward/reverse SVG reveals and a direct presentation URL at 760×1041. All 53 positional frames and 178 build checks pass at widescreen/4:3 sizes, plus 390px reading containment. All 18 topic breaks pass. Full Part II audit: 442 states and 656 formulas, with no overflow, equation failures or browser errors. Visually inspected the new sequence at 1280×720, 1024×768 and 760×1041, including intermediate gradient reveals. No trained model, notebook, dependency or Part III changes belong to this update.

## 2026-09-24 · Sine/cosine intuition before the clock

- Added one two-step frame, "Why sine and cosine?", between the sinusoidal method divider and the existing clock slider. A native unit-circle diagram compares indices 0 and 2 at the same illustrative 90-degree rate used by the following demo. Both points have sine zero. The reveal adds cosine and the complete pairs [1,0] and [-1,0]. The slide explains bounded coordinates and equal angular steps, while explicitly retaining the full-turn repetition caveat.
- Notes distinguish phase within a turn from an unlimited position identifier, cosine-first plotting from the standard sine-first formula, and additive position features from RoPE. The first-index convention is explicit. The following slide retains its working slider and repetition example. No model, notebook, dependency or Part III edits.
- Independent checks verify both point coordinates, their angles, equal sine values, different cosine values, displayed pairs, reverse/forward reveals and the next-frame transition. All 54 positional frames and 182 build checks pass, plus all 18 topic breaks. Full Part II audit: 444 states and 656 formulas, with no overflow, equation failures or browser errors. Visually inspected both reveals at 1280×720 and the finished frame at 1024×768 and 760×1041; automated checks also cover 1920×1080 and 390px reading containment.
