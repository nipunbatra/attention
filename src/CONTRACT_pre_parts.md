...
python3 assemble.py --only sections/sec07.html --out test07.html          # add --toy toy_synthetic.json ONLY if toy.json is absent
...
value from `AT` (`1×d_model = 1×4`). `opts: { groups: ['token','matrix','sizes','axes'], only: [latex symbols], into }`. The shell already places one in the hero
...
### `AT.ui.stepper(opts)` → `{ el, stage, list, go(i), next(), prev(), reset(), index(), buttons }`
...
Use it in s09, s11, s15, s18, s21 — same drawing every time; just change the active stage.
```js
...
- **No hard-coded toy numbers.** Everything numeric comes from `AT.model` / `AT.forward` / `AT.baseline`, except the SPEC's own illustrative examples (search table §5, `[2,5,9,1]` §12, 4×4 mask §13, labelled-illustrative multi-head patterns §17).
...
- **Keep notation exactly as BRIEF §3** (`e`, never `x/h/z`; `W_{\text{vocab}}`; `\Delta e`), and keep the Δ-notation disclaimer where the BRIEF requires it (s09, s11, s16, s20).
...
- Do not edit `shell.html`, `shared.js`, `katex-bundle.html`, `assemble.py`, `qa.mjs`, `toy.json`. If you need a component that is missing, build it fragment-locally with `AT.h` and the catalogue classes.
