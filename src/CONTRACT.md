...
`hidden` is the actual width, not a decorative node count. Widths above eight show numbered units 1–4, dots, and the final four units. Output columns insert dots at every omitted range, including both sides of a highlighted middle token. Edges connect only real displayed nodes, never the dots. Captions and accessible descriptions retain the full layer widths.
```js
