...
(function () {
  ...
  /* ---------- whiteboard helpers: one held drawing, one mark per build (STYLE_WHITEBOARD.md) ---------- */
  /* data-build on one element or a list (the runtime hides pending builds with visibility, so SVG marks and table rows work too) */
  V.onBuild = function (els, n) { (Array.isArray(els) || els instanceof NodeList ? Array.prototype.slice.call(els) : [els]).forEach(function (el) { if (el) el.setAttribute('data-build', String(n)); }); return els; };
  /* move SVG nodes into a <g data-build="n"> */
  V.buildGroup = function (sv, nodes, n) { var g = svg('g', { 'data-build': String(n) }); (nodes || []).forEach(function (el) { if (el) g.appendChild(el); }); sv.appendChild(g); return g; };
  /* a table drawn one row per build: row i gets build start + floor(i / perRow); the footer (if any) follows; returns the next free build */
  V.rowBuilds = function (fig, start, opts) {
    opts = opts || {}; var per = opts.perRow || 1, n = start == null ? 1 : start, last = n - 1;
    (fig.rowEls || []).forEach(function (tr, i) { last = n + Math.floor(i / per); tr.setAttribute('data-build', String(last)); });
    var next = last + 1;
    if (opts.footer !== false && fig.table && fig.table.tFoot) { var ftr = fig.table.tFoot.querySelector('tr'); if (ftr) { ftr.setAttribute('data-build', String(next)); next++; } }
    return next;
  };
  /* one 2x2 tile of plain numbers (squared errors, differences): tinted cells, no grey ramp */
  V.numTile = function (values, opts) {
    opts = opts || {}; var cell = opts.cell || 30, dec = opts.decimals == null ? 2 : opts.decimals;
    var s = svg('svg', { viewBox: '0 0 ' + 2 * cell + ' ' + 2 * cell, width: 2 * cell, height: 2 * cell, role: 'img', class: 'v6-tile v6-numtile' });
    values.forEach(function (v, i) {
      var r = Math.floor(i / 2), c = i % 2;
      s.appendChild(svg('rect', { x: c * cell, y: r * cell, width: cell, height: cell, fill: opts.fill || 'var(--t-a)', stroke: '#F7F8FA' }));
      s.appendChild(svg('text', { x: c * cell + cell / 2, y: r * cell + cell / 2 + 1, 'text-anchor': 'middle', 'dominant-baseline': 'middle', 'font-family': 'var(--font-mono)', 'font-size': Math.round(cell * .34), 'font-weight': 700, fill: opts.ink || 'var(--c-a)' }, AT.fmt(v, dec)));
    });
    s.appendChild(svg('rect', { x: .5, y: .5, width: 2 * cell - 1, height: 2 * cell - 1, fill: 'none', stroke: opts.stroke || 'var(--c-a)' }));
    var box = h('div', { class: 'vgrid' }, s); if (opts.caption) box = h('figure', { class: 'vfig', style: 'margin:0' }, box, h('figcaption', { html: opts.caption }));
    if (opts.into) opts.into.appendChild(box); return box;
  };
  /* a chain of boxes with arrows; items {text | tex | html, sub, cls, build, arrow} */
  V.chainRow = function (items, opts) {
    opts = opts || {}; var row = h('div', { class: 'v6-chain' + (opts.cls ? ' ' + opts.cls : '') });
    items.forEach(function (it, i) {
      if (i) { var ar = h('span', { class: 'v6-carr', 'aria-hidden': 'true' }, it.arrow || '→'); if (it.build != null) ar.setAttribute('data-build', String(it.build)); row.appendChild(ar); }
      var box = h('span', { class: 'v6-cbox' + (it.cls ? ' obj-' + it.cls : '') });
      var main = h('span', { class: 'v6-cmain' });
      if (it.tex) AT.tex(main, it.tex); else if (it.html) { main.innerHTML = it.html; AT.renderMath(main); } else main.textContent = it.text || '';
      box.appendChild(main);
      if (it.sub) box.appendChild(h('span', { class: 'v6-csub' }, it.sub));
      if (it.build != null) box.setAttribute('data-build', String(it.build));
      row.appendChild(box);
    });
    if (opts.into) opts.into.appendChild(row); return row;
  };
  /* a scatter with named axes, coordinates printed next to the points, and builds on points, arrows and the decision line.
     points {x, y, label, coords, cls:'e'|'ep'|'d'|'q'|'two'|'one', test, thumb:{scene,j}, build, dx, dy, anchor}; arrows {from, to, label, build}; line {w, b, label, build} */
  V.plot6 = function (points, opts) {
    opts = opts || {}; var W = opts.width || 520, H = opts.height || 340, m = { l: 64, r: 22, t: 18, b: 50 };
    var xs = points.map(function (p) { return p.x; }), ys = points.map(function (p) { return p.y; });
    (opts.arrows || []).forEach(function (ar) { xs.push(ar.from[0], ar.to[0]); ys.push(ar.from[1], ar.to[1]); });
    var xmin = Math.min.apply(null, xs), xmax = Math.max.apply(null, xs), ymin = Math.min.apply(null, ys), ymax = Math.max.apply(null, ys);
    if (opts.xlim) { xmin = opts.xlim[0]; xmax = opts.xlim[1]; } else { var px = (xmax - xmin) * .2 + .05; xmin -= px; xmax += px; }
    if (opts.ylim) { ymin = opts.ylim[0]; ymax = opts.ylim[1]; } else { var py = (ymax - ymin) * .3 + .05; ymin -= py; ymax += py; }
    var X = function (x) { return m.l + (x - xmin) / (xmax - xmin) * (W - m.l - m.r); }, Y = function (y) { return H - m.b - (y - ymin) / (ymax - ymin) * (H - m.t - m.b); };
    var s = svg('svg', { viewBox: '0 0 ' + W + ' ' + H, width: '100%', class: 'vscatter vplot6', style: 'max-width:' + W + 'px', role: 'img' });
    s.appendChild(svg('rect', { x: m.l, y: m.t, width: W - m.l - m.r, height: H - m.t - m.b, fill: 'var(--card)', stroke: 'var(--line)' }));
    if (xmin < 0 && xmax > 0) s.appendChild(svg('line', { x1: X(0), y1: m.t, x2: X(0), y2: H - m.b, stroke: 'var(--line)' }));
    if (ymin < 0 && ymax > 0) s.appendChild(svg('line', { x1: m.l, y1: Y(0), x2: W - m.r, y2: Y(0), stroke: 'var(--line)' }));
    var ax = opts.axes || ['axis 1', 'axis 2'], fs = opts.font || 15;
    s.appendChild(svg('text', { x: (m.l + W - m.r) / 2, y: H - 12, 'text-anchor': 'middle', 'font-size': fs + 1, fill: 'var(--ink)' }, ax[0]));
    s.appendChild(svg('text', { x: 18, y: (m.t + H - m.b) / 2, 'text-anchor': 'middle', 'font-size': fs + 1, fill: 'var(--ink)', transform: 'rotate(-90 18 ' + (m.t + H - m.b) / 2 + ')' }, ax[1]));
    [xmin, xmax].forEach(function (v) { s.appendChild(svg('text', { x: X(v), y: H - m.b + 17, 'text-anchor': 'middle', 'font-size': fs - 3, 'font-family': 'var(--font-mono)' }, AT.fmt(v, 1))); });
    [ymin, ymax].forEach(function (v) { s.appendChild(svg('text', { x: m.l - 7, y: Y(v) + 4, 'text-anchor': 'end', 'font-size': fs - 3, 'font-family': 'var(--font-mono)' }, AT.fmt(v, 1))); });
    if (opts.line && opts.line.w) {
      var w = opts.line.w, b = opts.line.b, pts = [];
      [xmin, xmax].forEach(function (x) { if (Math.abs(w[1]) > 1e-9) { var y = -(w[0] * x + b) / w[1]; if (y >= ymin && y <= ymax) pts.push([x, y]); } });
      [ymin, ymax].forEach(function (y) { if (Math.abs(w[0]) > 1e-9) { var x = -(w[1] * y + b) / w[0]; if (x > xmin && x < xmax) pts.push([x, y]); } });
      var lineG = svg('g'); if (opts.line.build != null) lineG.setAttribute('data-build', String(opts.line.build));
      if (pts.length >= 2) {
        var corners = [[xmin, ymin], [xmax, ymin], [xmax, ymax], [xmin, ymax]].filter(function (c) { return w[0] * c[0] + w[1] * c[1] + b > 0; });
        var poly = corners.concat(pts.slice(0, 2)).map(function (c) { return [X(c[0]), Y(c[1])]; });
        var cx = mean(poly.map(function (p) { return p[0]; })), cy = mean(poly.map(function (p) { return p[1]; }));
        poly.sort(function (a, b2) { return Math.atan2(a[1] - cy, a[0] - cx) - Math.atan2(b2[1] - cy, b2[0] - cx); });
        lineG.appendChild(svg('polygon', { points: poly.map(function (p) { return p.join(','); }).join(' '), fill: 'var(--t-e)', opacity: .55 }));
        lineG.appendChild(svg('line', { x1: X(pts[0][0]), y1: Y(pts[0][1]), x2: X(pts[1][0]), y2: Y(pts[1][1]), stroke: 'var(--ink)', 'stroke-width': 2.5, 'stroke-dasharray': '7 5' }));
        if (opts.line.label) lineG.appendChild(svg('text', { x: X(pts[1][0]) + (opts.line.dx || 8), y: Y(pts[1][1]) + (opts.line.dy || 16), 'font-size': fs - 1, fill: 'var(--ink)' }, opts.line.label));
      }
      s.appendChild(lineG);
    }
    var defs = svg('defs'), mid = 'v6p-' + (++hatchCount), mk = svg('marker', { id: mid, viewBox: '0 0 10 10', refX: 9, refY: 5, markerWidth: 7, markerHeight: 7, orient: 'auto' });
    mk.appendChild(svg('path', { d: 'M0 0L10 5L0 10Z', fill: 'var(--c-d)' })); defs.appendChild(mk); s.appendChild(defs);
    (opts.arrows || []).forEach(function (ar) {
      var g = svg('g'); if (ar.build != null) g.setAttribute('data-build', String(ar.build));
      g.appendChild(svg('line', { x1: X(ar.from[0]), y1: Y(ar.from[1]), x2: X(ar.to[0]), y2: Y(ar.to[1]), stroke: 'var(--c-d)', 'stroke-width': 2.5, 'marker-end': 'url(#' + mid + ')' }));
      if (ar.label) g.appendChild(svg('text', { x: (X(ar.from[0]) + X(ar.to[0])) / 2 + (ar.dx == null ? 6 : ar.dx), y: (Y(ar.from[1]) + Y(ar.to[1])) / 2 + (ar.dy == null ? -8 : ar.dy), 'font-size': fs - 1, fill: 'var(--c-d)' }, ar.label));
      s.appendChild(g);
    });
    points.forEach(function (p) {
      var gx = X(p.x), gy = Y(p.y), sz = p.r || 7, g = svg('g'); if (p.build != null) g.setAttribute('data-build', String(p.build));
      var el;
      if (p.thumb) { el = V.thumb(p.thumb.scene, p.thumb.j, { size: 18 }); el.setAttribute('x', gx - 9); el.setAttribute('y', gy - 9); }
      else if (p.cls === 'two') el = svg('rect', { x: gx - sz, y: gy - sz, width: 2 * sz, height: 2 * sz, fill: 'var(--c-e)', stroke: p.test ? 'var(--warn)' : '#fff', 'stroke-width': p.test ? 3 : 1.5 });
      else if (p.cls === 'one') el = svg('circle', { cx: gx, cy: gy, r: sz, fill: 'var(--card)', stroke: p.test ? 'var(--warn)' : 'var(--ink)', 'stroke-width': p.test ? 3 : 2 });
      else el = svg('circle', { cx: gx, cy: gy, r: sz, fill: p.cls === 'd' ? 'var(--c-d)' : p.cls === 'q' ? 'var(--c-q)' : 'var(--c-e)', stroke: p.cls === 'ep' ? 'var(--c-d)' : '#fff', 'stroke-width': p.cls === 'ep' ? 3 : 1.5 });
      el.appendChild(svg('title', {}, (p.label || '') + ' (' + AT.fmt(p.x, 2) + ', ' + AT.fmt(p.y, 2) + ')')); g.appendChild(el);
      var lx = gx + (p.dx == null ? sz + 5 : p.dx), ly = gy + (p.dy == null ? 5 : p.dy), anchor = p.anchor || 'start';
      if (p.label) g.appendChild(svg('text', { x: lx, y: ly, 'font-size': fs, 'font-weight': 600, fill: 'var(--ink)', 'text-anchor': anchor }, p.label));
      if (p.coords) g.appendChild(svg('text', { x: lx, y: ly + fs, 'font-size': fs - 2, 'font-family': 'var(--font-mono)', fill: 'var(--ink-2)', 'text-anchor': anchor }, '(' + AT.fmt(p.x, 2) + ', ' + AT.fmt(p.y, 2) + ')'));
      s.appendChild(g);
    });
    var box = h('div', { class: 'vfig' }, s); if (opts.caption) box.appendChild(h('figcaption', { html: opts.caption }));
    box.svg = s; box.X = X; box.Y = Y;
    if (opts.into) opts.into.appendChild(box); return box;
  };
  /* present mode: cards are read-mode containers; on the stage the drawing stands alone */
  document.head.appendChild(h('style', {}, [
    'body.present .frame > .card{padding:0;border:0;background:transparent;box-shadow:none;margin:0 0 12px}',
    'body.present .frame > .card > .card-title{margin:0 0 10px}',
    '.v6-line{font-size:1.02em;line-height:1.45;margin:10px 0 0;max-width:76ch}',
    'body.present .v6-line{font-size:var(--present-body);line-height:1.35;margin:14px 0 0;max-width:none}',
    '.v6-chain{display:flex;flex-wrap:wrap;gap:8px 10px;align-items:center;margin:6px 0}',
    '.v6-cbox{display:inline-flex;flex-direction:column;align-items:center;gap:1px;border:2px solid var(--oc,var(--line));background:var(--ot,var(--card));border-radius:10px;padding:7px 13px;font-weight:600;color:var(--ink);line-height:1.2}',
    '.v6-csub{font-size:.72em;font-weight:500;color:var(--ink-2);white-space:nowrap}',
    '.v6-carr{color:var(--ink-3);font-size:1.25em;line-height:1}',
    'body.present .v6-cbox{font-size:25px;padding:9px 15px}',
    'body.present .v6-chain{gap:10px 12px}',
    '.v6-tiles{display:flex;flex-wrap:wrap;gap:18px 30px;align-items:flex-start}',
    '.v6-tiles figcaption{font-size:14px;color:var(--ink-2);margin-top:6px;text-align:center}',
    'body.present .v6-tiles figcaption{font-size:22px}',
    '.v6-op{font-size:1.5em;color:var(--ink-2);align-self:center;padding:0 2px;font-weight:600}',
    'body.present .v6-op{font-size:1.7em}',
    '.v6-calc .calc-line{margin:4px 0 0;font-family:var(--font-mono);font-variant-numeric:tabular-nums}',
    '.v6-calc .calc-rl{display:inline-block;min-width:6.2em;text-align:right;margin-right:8px;font-family:var(--font-ui);font-weight:600;color:var(--ink-2)}',
    'body.present .v6-calc .calc-line{font-size:23px;line-height:1.5}',
    '.v6-margin{font-size:15px;color:var(--ink-2);line-height:1.45;margin:0;padding-left:1.1em}',
    '.v6-margin li{margin:4px 0}',
    'body.present .v6-margin{font-size:22px;line-height:1.35}',
    '.v6-draw svg{display:block;width:100%;height:auto}',
    '.v6-draw text{font-family:var(--font-ui)}'
  ].join('\n')));
