...
Parts 2 and 3 share the exact 4→8→20 ReLU prediction MLP. Part 3 is a 39-frame visual story: work through shared versus independent source mixtures, recall Part II’s Maya query/key/value example, compute each head’s query, then trace the river-bank messages and matrix shapes. It retains Part II’s E, A, H, M, ΔE and E′ notation, separates concatenation from the output projection, and explains projection biases beside the code. Notebook 7 embeds those same figures before an optional 58-step executable lab, including scratch/PyTorch parity checks. A separate three-seed TinyStories experiment compares genuinely trained MLP, one-head and four-head models. Normalization, full Transformer blocks and complexity remain in optional Part 2B.
