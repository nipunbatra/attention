...
Parts 2 and 3 share the exact 4→8→20 ReLU prediction MLP. Part 3 is a 34-frame visual story: see two reading patterns on the same river-bank sentence, work out their messages, compare them with one wider head, combine them, then trace the matrix shapes. It retains Part II’s E, A, H, M, ΔE and E′ notation and explains projection biases beside the code. Notebook 7 embeds those same figures before an optional 58-step executable lab, including scratch/PyTorch parity checks. A separate three-seed TinyStories experiment compares genuinely trained MLP, one-head and four-head models. Normalization, full Transformer blocks and complexity remain in optional Part 2B.
